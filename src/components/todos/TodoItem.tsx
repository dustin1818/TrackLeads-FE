    ...
    <div className="flex flex-col gap-3 rounded-lg border bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <p className={todo.isCompleted ? 'text-slate-400 line-through' : 'text-slate-800'}>{todo.title}</p>
          <Badge className={priorityStyles[todo.priority]}>{todo.priority}</Badge>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="destructive" size="sm" onClick={() => onDelete(todo._id)}>
          Delete
        </Button>
      </div>
    </div>
